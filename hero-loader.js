(() => {
  ...
  const buildMobileBlob = (parts) => {
    // The mobile artwork was uploaded as binary slices that were base64-encoded
    // independently. Concatenating those base64 strings before decoding can still
    // produce a technically decodable WebP while corrupting macroblocks on Android.
    // Decode each slice first, then let Blob concatenate the original bytes.
    if (canDecodeSeparately(parts)) {
      return new Blob(parts.map(decodeBase64), { type: 'image/webp' });
    }

    // Backward-compatible path if the files are ever regenerated by splitting one
    // continuous base64 string instead of encoding binary slices independently.
